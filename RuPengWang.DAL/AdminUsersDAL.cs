...
namespace RuPengWang.DAL
{
	...
	public partial class AdminUsersDAL
	{
		...
		public RuPengWang.Model.AdminUsers GetModel(long Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,UserName,Password from T_AdminUsers ");
			strSql.Append(" where Id=@Id");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.BigInt)
			};
			...
			RuPengWang.Model.AdminUsers model=new RuPengWang.Model.AdminUsers();
		...
		/// 得到一个对象实体
		/// </summary>
		public RuPengWang.Model.AdminUsers DataRowToModel(DataRow row)
		{
			RuPengWang.Model.AdminUsers model=new RuPengWang.Model.AdminUsers();
			if (row != null)
			{
